import React from 'react';
import robotImg from '@/assets/images/robot.svg';
import prevButtonImg from '@/assets/images/prevButton.svg';
import nextButtonImg from '@/assets/images/nextButton.svg';
// import bookcover from '@/assets/images/bookcover.svg';

function FrontCoverPage() {
  return (
    <div className="w-screen h-screen bg-mainColor bg-opacity-15 relative z-10">
      <div className="flex flex-row">
        <img className="flex w-20 z-20 ml-10" src={prevButtonImg} alt="prev_button" />
        <div className="flex flex-col basis-1/2 h-screen justify-center">
          <div className="flex basis-1/2 justify-center">
            <img className="flex mx-auto min-h-full z-20 w-80 mt-28" src={robotImg} alt="robot_character" />
          </div>
          <div className="flex flex-col basis-1/2 items-center align-middle mt-24 ml-5 gap-y-10">
            <div className="flex justify-center font-dongle text-7xl w-2/3 leading-snug break-keep">
              라푼젤의 주인공이 되어 이야기를 완성시켜봐!
            </div>
          </div>
        </div>
        <div className="flex basis-1/2 h-screen justify-center content-center">
          <div className="flex bg-bookCoverBack h-[90%] w-[97%] mt-8 -ml-24 rounded-3xl">
            <div className="flex flex-col bg-bookCoverFront h-full w-full mt-6 z-10 -ml-6 mr-6 rounded-3xl justify-center items-center">
              <div className="bg-bookCoverTextBox h-[35%] w-4/5 mt-20 rounded-3xl border-2 border-shadowGray border-solid font-dongle text-8xl mb-44">
                <div className="h-full w-full justify-center p-10 break-keep">라푼젤보다 조금 더 긴 제목 테스트</div>
              </div>
              <div className="bg-bookCoverLine h-1/6 w-full mb-44"></div>
            </div>
          </div>
        </div>
        <img className="w-20 z-20 mr-10 -ml-28" src={nextButtonImg} alt="next_button" />
      </div>
    </div>
  );
}

export default FrontCoverPage;
